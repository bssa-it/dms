...
class CRM_Contribute_Selector_Search extends CRM_Core_Selector_Base implements CRM_Core_Selector_API {
  ...
  function &getRows($action, $offset, $rowCount, $sort, $output = NULL) {
    ...
    While ($result->fetch()) {
      ...
      # DMS fields
      $row['payment_method'] = $result->payment_instrument_id;
      
      $rParams = array("contribution_id" => $result->contribution_id);
      $reportingCodes = CRM_Dmsextension_BAO_Transaction::getValues($rParams);
      
      $row['motivation_id'] = (count($reportingCodes)===1) ? $reportingCodes[0]['motivation_id']:'';
      $row['category_id'] = (count($reportingCodes)===1) ? $reportingCodes[0]['category_id']:'';
      $row['organisation_id'] = (count($reportingCodes)===1) ? $reportingCodes[0]['organisation_id']:'';
      $row['region_id'] = (count($reportingCodes)===1) ? $reportingCodes[0]['region_id']:'';
      # END DMS fields
