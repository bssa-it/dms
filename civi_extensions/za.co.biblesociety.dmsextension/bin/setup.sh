...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Dmsextension/DAO/" ] && mkdir -p "$EXTROOT/CRM/Dmsextension/DAO/"
  cp -f "$CIVIROOT/CRM/Dmsextension/DAO"/* "$EXTROOT/CRM/Dmsextension/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Dmsextension" "$EXTROOT/CRM/Dmsextension/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
