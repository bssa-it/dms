...
/**
 * @description
 * This script inserts a task into zimbra.
 * 
...
$taskListType = $_POST['taskListType'];
$taskListId = (isset($_POST['taskListId'])) ? 'l="'.$_POST['taskListId'].'"':'';
$startTime = date("Ymd",strtotime($_POST['taskDateTime'])) . 'T' . date("His",strtotime($_POST['taskDateTime']));
$endTime = date("Ymd",strtotime($_POST['taskDateTime'])) . 'T' . date("His",strtotime($_POST['taskDateTime'] . ' + 30 minutes'));
$taskDetails = (isset($_POST['details'])) ? $_POST['details']:'';
$department = (isset($_POST['department'])) ? $_POST['department']:'';
$subject = $_POST['subject'];
$organizer = '';

if ($taskListType=='S'&&!empty($department)) {
    $d = new department;
    $d->Load($department);
...
<m '.$taskListId.'>
    ...
    <mp ct="text/html"><content>'.$taskDetails.'</content></mp>
</m>';
...
try {
    ...
    try {
        if (strpos($subject,"dms-")>0) {
            ...
            $taskResponse = $body[0]->CreateTaskResponse;
            $itemId = (string)$taskResponse['calItemId'];
